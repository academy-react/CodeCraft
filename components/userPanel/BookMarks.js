...
const BookMarks = (props) => {
  ...
  const [mainCourses, setMainCourses] = useState(contextData.bookList);
  ...
  useEffect(() => {
    let updatedData = contextData.bookList;
    if (courseSearch) {
  ...
  useEffect(() => {
    setMainCourses(contextData.bookList);
  }, [contextData.bookList]);
    ...
    <>
      <div className="md:p-0 sm:p-5">
        ...
        <p className="block w-full  text-center mt-3 dark:text-gray-300 text-gray-400 text-lg">
          متاسفانه در سایت ما برای هر دوره برای پشتیبانی بهتر ظرفیت محدودی دارد
          پس بهتر است هر چه سریع تر در دوره مورد علاقه خود تا قبل از افزایش
          ظرفیت ثبت نام کنید
        </p>
                ...
                mainCourses
                  ...
                  .map((course) => (
                    <Course
                      ...
                      key={course.id}
                      view={selectedView}
                      handleDeleteCourse={contextData.handleDeleteFromBookList}
                      bookList
                    />
                  ))
              ) : (
