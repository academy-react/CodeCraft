...
const WatingPage = (props) => {
  const contextData = useContext(mainContext);
  const itemsPerPage = useRef();

  const [mainCourses, setMainCourses] = useState(
    contextData.waitingPageCourses
  );
  ...
  useEffect(() => {
    let updatedData = contextData.waitingPageCourses;
    if (courseSearch) {
  ...
  useEffect(() => {
    setMainCourses(contextData.waitingPageCourses);
  }, [contextData.waitingPageCourses]);
  ...
  return (
    <>
      <div className="md:p-0 sm:p-5">
        ...
        <p className="block w-full  text-center mt-3 dark:text-gray-300 text-gray-400 text-lg">
          متاسفانه در سایت ما هر دوره ظرفیت محدودی برای ثبت نام دارد دوره هایی
          که در صفحه انتظار شما است در صورت افزایش ظرفیت توسط مدرس به سبد خرید
          شما انتقال داده خواهد شد
        </p>
                ...
                mainCourses
                  ...
                  .map((course) => (
                    <Course
                      ...
                      view={selectedView}
                      handleDeleteCourse={
                        contextData.handleDeleteFromWaitingPage
                      }
                      myList
                    />
                  ))
